...
def ip_parser():
    if sys.platform in ('linux', 'linux2'):
        ...
        conf_check = list(map(lambda x: re.search(r'[\w\.]+', re.search(r'\(.*\)', re.search(r'RX packets.*\(.*\)', x).group()).group()).group(), conf))
    ...
    if sys.platform == 'win32':
        ...
        ip = re.search(r':.*', re.search(r'IPv4.*:.*\..*', conf).group()).group()[2:]
    ext_ip = subprocess.Popen('nslookup myip.opendns.com. resolver1.opendns.com', stdout=subprocess.PIPE, shell=True).communicate()[0].decode()
